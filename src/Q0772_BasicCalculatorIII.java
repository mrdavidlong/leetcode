...
public class Q0772_BasicCalculatorIII {
    public int calculate(String s) {
        int l1 = 0, o1 = 1;
        int l2 = 1, o2 = 1;
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (Character.isDigit(c)) {
                ...
                // + or - to the level 1 values from level 2 value
                l1 = l1 + o1 * l2;

                o1 = (c == '+' ? 1 : -1);

                // reset the level 2 defaults
                l2 = 1; o2 = 1;
            }
        }
    ...
    public static void main(String[] args) {
        Q0772_BasicCalculatorIII sol = new Q0772_BasicCalculatorIII();
        int result = sol.calculate("1 + 1"); // 2
        int result2 = sol.calculate(" 6-4 / 2 "); // 4
        int result3 = sol.calculate("2*(5+5*2)/3+(6/2+8)"); // 21
        int result4 = sol.calculate("(2+6* 3+5- (3*14/7+2)*5)+3"); // -12
    }
}
